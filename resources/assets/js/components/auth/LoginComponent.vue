<template>
    <div class="login__block active" id="l-login">
        ...
        <div class="login__block__body">
            <form class="form-horizontal" :action="pUrlLogin" @submit.prevent="fOnSubmitLogin($event, $validator, user)" novalidate>
                ...
                <div class="form-group form-group--float form-group--centered">
                    <input type="password" id="password" name="password" data-vv-as="Senha" v-model="user.password" v-bind:class="{'form-control': true, 'is-invalid': this.errors.has('password')}" v-validate="'required'">
                    <span v-bind:class="{'invalid-feedback': this.errors.has('password')}">{{ this.errors.first('password') }}</span>
                    <label for="password">Password</label>
                    <i class="form-group__bar"></i>
                </div>

                <button type="submit" class="btn btn--icon login__block__btn" :disabled="this.errors.any()"> <i class="zmdi zmdi-long-arrow-right"></i> </button>
            </form>
        </div>
    </div>
</template>
...
<script>
    export default {
    	props: ['pUrlLogin'],
        methods: {
	        fOnSubmitLogin: (oEvent, oValidator, oUser) => {
